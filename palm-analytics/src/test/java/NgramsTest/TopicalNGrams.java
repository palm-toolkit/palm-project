...
import cc.mallet.util.Randoms;
	...
	public InstanceList ilist; // containing FeatureSequenceWithBigrams in the
		...
		// Used for printing both unigrams and ngrams 
		...
		// prints unigrams 
	  public String printUnigrams(int numWords, boolean weight){
		...
		for (int ti = 0; ti < numTopics; ti++) {
			...
			for (int i = 0; i < numToPrint; i++)
				if (weight){
					...
					out.append(uniAlphabet.lookupObject(wp[i].wi).toString() + " ");
				}
			out.append( "\n" );
		}
		  return out.toString();
	  ...
	  // print Ngrams
	  public String printNgrams(int numWords, boolean weight){
		 StringBuilder out = new StringBuilder();
		  class WordProb implements Comparable {
				int wi; double p;
				public WordProb (int wi, double p) { this.wi = wi; this.p = p; }
...
//			                  +" phrases "+Math.round(afv.oneNorm())+"/"+numNgrams+")\n         ");
			...
			for (int i = 0; i < Math.min(numNgrams, numWords); i++)
				if (weight){
					out.append (afv.getAlphabet().lookupObject(wp[i].wi).toString() + "-" + wp[i].p/ngramSum + " ");
				} else {
					out.append (afv.getAlphabet().lookupObject(wp[i].wi).toString() + " ");
				}
			out.append("\n");
			}
	  ...
	  // use delimiter = ^ later if weight= true, 
	  public String documentTopics (double threshold, int max, boolean weight)
	  {
		  	StringBuilder out = new StringBuilder();
		    int docLen;
		    double topicDist[] = new double[topics.length];
		     ...
		     // out.append (di); out.append (" ");
		      ...
		      for (int tp = 0; tp < numTopics; tp++) {
		        ...
		        if (maxindex == -1 || topicDist[maxindex] < threshold)
		          break;
		        if (weight){
		        	out.append (maxindex + " ^ " + topicDist[maxindex]+" ");
		        } else {
		        	out.append (maxindex+" ");
		        }
		        topicDist[maxindex] = 0;

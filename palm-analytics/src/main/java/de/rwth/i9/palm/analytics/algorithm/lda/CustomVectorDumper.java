 ...
 * and dump out the results using {@link Vector#asFormatString()} to either the
 * console or to a file.
 */
public class CustomVectorDumper extends AbstractJob
{

	private static final Logger log = LoggerFactory.getLogger( CustomVectorDumper.class );

	public CustomVectorDumper()
	{
	}
	...
	public int run( String[] args ) throws Exception
	{
		...
		try
		{
			...
			int maxIndexesPerVector = hasOption( "vectorSize" ) ? Integer.parseInt( getOption( "vectorSize" ) ) : Integer.MAX_VALUE;
			long itemCount = 0;
			int fileCount = 0;
			for ( Path path : pathArr )
			{
				...
				while ( iterator.hasNext() && ( numItems == null || itemCount < numItems ) )
				{
					...
					if ( filters != null && vector instanceof NamedVector && !filters.contains( ( (NamedVector) vector ).getName() ) )
					{
					...
					else
					{
						...
						{
							fmtStr = VectorHelper.vectorToJson( vector, dictionary, maxIndexesPerVector, sortVectors );
						}
						writer.write( fmtStr );
						writer.write( '\n' );
					}
	...
	{
		ToolRunner.run( new Configuration(), new CustomVectorDumper(), args );
	}

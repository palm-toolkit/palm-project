...
import de.rwth.i9.palm.analytics.algorithm.dynamicLDA.WorkerRunnable;
...
public class ParallelTopicModel implements Serializable {
	...
	public String printoneDocumentTopics (double threshold, int max, int docID)	{
		int docLen;
		int[] topicCounts = new int[ numTopics ];
		...
		if (data.get(docID).instance.getName() != null) {
			builder.append(data.get(docID).instance.getName()); 
		}
		else {
			builder.append("no-name");
		}
	...
	public String modifiedprintTopicDocuments (int max)	{
		StringBuilder out = new StringBuilder();
		out.append("#topic doc name proportion ...\n");
